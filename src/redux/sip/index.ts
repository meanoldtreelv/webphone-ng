...
const sipSlice = createSlice({
	...
	initialState: {
		...
		audioAutoGainControl: localStorage.getItem("audioAutoGainControl")
			? localStorage.getItem("audioAutoGainControl") == "true"
			: true,
		audioNoiseSuppression: localStorage.getItem("audioNoiseSuppression")
			? localStorage.getItem("audioNoiseSuppression") == "true"
			: true,
		audioEchoCancellation: localStorage.getItem("audioEchoCancellation")
			? localStorage.getItem("audioEchoCancellation") == "true"
			: true,
		...
		ringingInboundCalls: (state, action) => {
			switch (action.payload.action) {
				...
				case "remove": {
					...
					if (
						state.ringingInboundCallActive === lineNum &&
						state.ringingInboundCalls.length > 0 &&
						state.ringingInboundCalls[0].LineNumber !== undefined
					) {
						//Inbound Call
		...
		answeredCalls: (state, action) => {
			switch (action.payload.action) {
				...
				case "remove": {
					...
					for (let index = 0; index < state.answeredCalls.length; index++) {
						if (state.answeredCalls[index].LineNumber === lineNum) {
							const endingCall = {
								name: state.answeredCalls[index].DisplayName,
								callTimer: state.answeredCalls[index].callTimer,
								number: state.answeredCalls[index].DisplayNumber,
							};
							state.callEnding = [...state.callEnding, endingCall];
							state.answeredCalls = [...state.answeredCalls.slice(0, index), ...state.answeredCalls.slice(index + 1)];
							break;
						}
					}
					if (
						...
						state.answeredCalls[0].LineNumber !== undefined
					) {
						//Answered
						state.answeredCallActive = state.answeredCalls[0].LineNumber;
						state.activeCallLineNumber = state.answeredCalls[0].LineNumber;
				...
				case "addConferenceCall": {
					...
					for (let index = 0; index < state.answeredCalls.length; index++) {
						if (state.answeredCalls[index].LineNumber === lineNum) {
							state.answeredCalls[index].conferenceCallList = [
								...(state.answeredCalls[index].conferenceCallList ?? []),
								conferenceCallList,
							];
		...
		ringingOutboundCalls: (state, action) => {
			switch (action.payload.action) {
				...
				case "remove": {
					...
					console.log(state.ringingOutboundCalls);
					if (
						state.ringingOutboundCallActive === lineNum &&
						state.ringingOutboundCalls.length > 0 &&
						state.ringingOutboundCalls[0].LineNumber !== undefined
					) {
						//Outbound Call
						state.ringingOutboundCallActive = state.ringingOutboundCalls[0].LineNumber;
						state.activeCallLineNumber = state.ringingOutboundCalls[0].LineNumber;
