...
// clio URLs
const getClioCallBackUrl = () => {
	let url = "";
	if (isLocalhost) {
		url = "http://127.0.0.1:3000/clio";
	}
	if (isDev) {
		url = "https://webphone.dev.ringplan.com/clio";
	}
	if (isProd) {
		url = "https://webphone.ringplan.com/clio";
	}
	// url = window.location.origin + "/clio";
	return url;
};
