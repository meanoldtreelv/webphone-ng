...
let AutoGainControl = () => {
	return getCookie("audioAutoGainControl") ? getCookie("audioAutoGainControl") == "true" : true;
}; // Attempts to adjust the microphone volume to a good audio level. (OS may be better at this)
let EchoCancellation = () => {
	return getCookie("audioEchoCancellation") ? getCookie("audioEchoCancellation") == "true" : true;
}; // Attempts to remove echo over the line.
let NoiseSuppression = () => {
	return getCookie("audioNoiseSuppression") ? getCookie("audioNoiseSuppression") == "true" : true;
...
function getAudioSrcID() {
	if (
		getCookie("microphoneDevice") &&
		getCookie("microphoneDevice") != "" &&
		getCookie("microphoneDevice") != "default"
	) {
	...
	try {
		if ("Notification" in window) {
			if (Notification.permission === "granted") {
				...
				var inComingCallNotification = new Notification("incoming_call", noticeOptions);
