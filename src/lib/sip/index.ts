...
let DoNotDisturbEnabled = () => {
	return store.getState().sip.status.main_status ? store.getState().sip.status.main_status == "do_not_disturb" : false;
}; // Rejects any inbound call, while allowing outbound calls
...
let AutoGainControl = () => {
	return localStorage.getItem("audioAutoGainControl") ? localStorage.getItem("audioAutoGainControl") == "true" : true;
}; // Attempts to adjust the microphone volume to a good audio level. (OS may be better at this)
let EchoCancellation = () => {
	return localStorage.getItem("audioEchoCancellation") ? localStorage.getItem("audioEchoCancellation") == "true" : true;
}; // Attempts to remove echo over the line.
let NoiseSuppression = () => {
	return localStorage.getItem("audioNoiseSuppression") ? localStorage.getItem("audioNoiseSuppression") == "true" : true;
}; // Attempts to clear the call quality of noise.
...
function getAudioSrcID() {
	if (
		localStorage.getItem("microphoneDevice") &&
		localStorage.getItem("microphoneDevice") != "" &&
		localStorage.getItem("microphoneDevice") != "default"
	) {
...
function CreateUserAgent() {
	...
	if (SipDomain === null || SipDomain === "" || SipDomain === "null" || SipDomain === "undefined")
...
function onTransportConnectError(error) {
	...
	// Custom Web hook
	if (typeof onTransportError !== "undefined") onTransportError(userAgent.transport, userAgent);
}
		...
		requestDelegate: {
			onAccept: function () {
				...
				store.dispatch({
					...
					payload: { action: "isHold", data: { lineNum: lineNum, isHold: true } },
				});
			},
			onReject: function () {
	...
	var options = {
		requestDelegate: {
			onAccept: function () {
				...
				store.dispatch({
					type: "sip/answeredCalls",
					payload: { action: "isHold", data: { lineNum: lineNum, isHold: false } },
				});
...
function MuteSession(lineNum: number) {
	...
	store.dispatch({
		type: "sip/answeredCalls",
		payload: { action: "isMute", data: { lineNum: lineNum, isMute: true } },
	});
	store.dispatch({
		type: "sip/ringingOutboundCalls",
		payload: { action: "isMute", data: { lineNum: lineNum, isMute: true } },
	});
...
function UnmuteSession(lineNum: number) {
	...
	store.dispatch({
		type: "sip/answeredCalls",
		payload: { action: "isMute", data: { lineNum: lineNum, isMute: false } },
	});
	store.dispatch({
		type: "sip/ringingOutboundCalls",
		payload: { action: "isMute", data: { lineNum: lineNum, isMute: false } },
...
function sendDTMF(lineNum: number, itemStr: string) {
	...
	if (lineObj.SipSession.isOnHold === true) {
		if (lineObj.SipSession.data.childsession) {
			...
			console.warn(
				"Cannot Send DTMF (" + itemStr + "): " + lineObj.LineNumber + " is on hold, and there is no child session",
			);
		}
	} else {
		if (
			...
			console.log("#line-" + lineNum + "-msg:" + "send_dtmf" + ": " + "itemStr");

			// Custom Web hook
			if (typeof onDTMF !== "undefined") onDTMF(itemStr, lineNum);
		} else {
			console.warn(
				"Cannot Send DTMF (" + itemStr + "): " + lineObj.LineNumber + " session is not establishing or established",
			);
		}
	}
}
...
function onSessionReceivedBye(lineObj, response) {
	...
	lineObj.SipSession.data.reasonCode = 16;
	lineObj.SipSession.data.reasonText = "Normal Call clearing";
	lineObj.SipSession.disposed = true;
				...
				remoteAudio
					...
					.then(function () {
						...
						store.dispatch({
							type: "sip/answeredCalls",
							payload: {
								action: "callSpeakerDevice",
								data: { lineNum: lineObj.LineNumber, callSpeakerDevice: getAudioOutputID() },
							},
						});
					})
					.catch(function (e) {
			...
			for (var i = 0; i < ci.length; i++) {
				...
				if (raw_ci.indexOf("answer-after=") > 0) {
					var temp_seconds_autoanswer = parseInt(
						raw_ci.substring(raw_ci.indexOf("answer-after=") + "answer-after=".length).split(";")[0],
					);
		...
		if (autoAnswerRequested === false && ai !== undefined && ai.length > 0) {
			for (var i = 0; i < ai.length; i++) {
				...
				if (raw_ai.indexOf("answer-after=") > 0) {
					var temp_seconds_autoanswer = parseInt(
						raw_ai.substring(raw_ai.indexOf("answer-after=") + "answer-after=".length).split(";")[0],
					);
					if (Number.isInteger(temp_seconds_autoanswer) && temp_seconds_autoanswer >= 0) {
						autoAnswerRequested = true;
						if (temp_seconds_autoanswer > 1) answerTimeout = temp_seconds_autoanswer * 1000;
		...
		if ("Notification" in window) {
			Notification.requestPermission().then((permission) => {
				if (Notification.permission === "granted") {
					var noticeOptions = {
						body: "Incoming call from " + callerID + " <" + did + ">",
						icon: "./../../assets/images/core/logo-ri-fill.svg",
					};
...
function onInviteAccepted(lineObj, includeVideo, response) {
	...
	session.data.callTimer = window.setInterval(function () {
		...
		store.dispatch({
			type: "sip/answeredCalls",
			payload: { action: "callTimer", data: { lineNum: lineObj.LineNumber, callTimer: timeStr } },
		});
	...
	store.dispatch({
		type: "sip/answeredCalls",
		payload: {
			action: "callMicrophoneDevice",
			data: { lineNum: lineObj.LineNumber, callMicrophoneDevice: lineObj.SipSession.data.AudioSourceDevice },
		},
	});
}
...
function onInviteProgress(lineObj, response) {
	...
	if (response.message.statusCode === 180) {
		...
		console.log("#line-" + lineObj.LineNumber + "-msg:" + response.message.reasonPhrase + "...");
	} else {
		// 181 = Call is Being Forwarded
		// 182 = Call is queued (Busy server!)
		// 199 = Call is Terminated (Early Dialog)

		consloe.log("#line-" + lineObj.LineNumber + "-msg" + response.message.reasonPhrase + "...");
	}
	...
	var transferOptions = {
		requestDelegate: {
			onAccept: function (sip) {
				...
				console.log("#line-" + lineNum + "-msg" + "Call Blind Transferred (Accepted)");
...
function AttendedTransfer(lineNum: number, dstNo: string) {
	...
	newSession.delegate = {
		onBye: function (sip) {
			...
			console.log("#line-" + lineNum + "-msg" + "attended_transfer_call_terminated");
		},
		onSessionDescriptionHandler: function (sdh, provisional) {
			if (sdh) {
				if (sdh.peerConnection) {
	...
	var inviterOptions = {
		requestDelegate: {
			onTrying: function (sip) {
				...
				console.log("#line-" + lineNum + "-msg:" + "attended_transfer_call_started");
			},
			onProgress: function (sip) {
			...
			onAccept: function (sip) {
					...
					var transferOptions = {
						requestDelegate: {
							onAccept: function (sip) {
								...
								console.log("#line-" + lineNum + "-msg:" + "attended_transfer_complete_accepted");
								...
								if (!session.data.confcalls) {
									store.dispatch({
										type: "sip/answeredCalls",
										payload: { action: "remove", data: lineObj.LineNumber },
									});
								}
								teardownSession(lineObj);
								SelectLine(store.getState().sip.activeCallLineNumber);
							...
							onReject: function (sip) {
								...
								console.log("#line-" + lineNum + "-msg:" + "Attended Transfer Failed!");
							},
						},
					};
...

				{
					...
					console.log("#line-" + lineNum + "-msg:" + "attended_transfer_call_ended");
			...
			onReject: function (sip) {
				...
				console.log("#line-" + lineNum + "-msg:" + "attended_transfer_call_rejected");
			},
		},
	};
	...
	newSession.delegate = {
		onBye: function (sip) {
			...
			store.dispatch({
				type: "sip/answeredCalls",
				payload: {
					action: "conferenceCallDisposition",
					data: { confCallId: confCallId, lineNum: lineNum, disposition: "Bye" },
				},
			});
		...
		requestDelegate: {
			onTrying: function (sip) {
				...
				store.dispatch({
					...
					payload: {
						action: "conferenceCallDisposition",
						data: { confCallId: confCallId, lineNum: lineNum, disposition: "Trying" },
					},
				});
			},
			onProgress: function (sip) {
				...
				store.dispatch({
					...
					payload: {
						action: "conferenceCallDisposition",
						data: { confCallId: confCallId, lineNum: lineNum, disposition: "Progress" },
					},
				});
			},
			onRedirect: function (sip) {
			...
			onAccept: function (sip) {
				...
				// Join Call
				{
					...
					console.log("#line-" + lineNum + "-btn-terminate-conference-call show()");
					console.log("#line-" + lineNum + "-msg" + "conference_call_in_progress");
					...
					session.data.confcalls[confCallId].callTimer = window.setInterval(function () {
						...
						store.dispatch({
							type: "sip/answeredCalls",
							payload: {
								action: "conferenceCallTimer",
								data: { confCallId: confCallId, lineNum: lineNum, callTimer: timeStr },
							},
						});
					}, 1000);
					store.dispatch({
						...
						payload: {
							action: "conferenceCallDisposition",
							data: { confCallId: confCallId, lineNum: lineNum, disposition: "Joined" },
						},
					});
					// window.clearInterval(session.data.confcalls[confCallId].callTimer);
				}
				...
				store.dispatch({
					...
					payload: {
						action: "conferenceCallDisposition",
						data: { confCallId: confCallId, lineNum: lineNum, disposition: "Accepted" },
					},
				});
			},
			onReject: function (sip) {
				...
				store.dispatch({
					...
					payload: {
						action: "conferenceCallDisposition",
						data: { confCallId: confCallId, lineNum: lineNum, disposition: sip.message.reasonPhrase },
					},
				});
			},
		},
...
function mute(id, status_interface = null, update_interface = true) {
	...
	if ((update_interface === true && id === getCurrentActiveCallId()) || status_interface !== null) {
		...
		$(".webphone-mute-btn > img").attr("src", _vSansayUiMute ? "/images/unmute.svg" : "/images/mute.svg");
...
function keyPressed(key, isTriggerOff) {
	...
	if ($(".webphone-digits").val().length > 0) {
		document.querySelector("#webphone-backspace-btn").classList.remove("hidden");
	}
...
function renderMicrophoneDevice() {
	try {
		let microphoneDevice = removeAllOtionFromSelectInput(microphoneDevice); //document.getElementById("microphoneDevice");
...
function renderSpeakerDevice() {
	try {
		let speakerDevice = removeAllOtionFromSelectInput(speakerDevice); //document.getElementById("speakerDevice");
...
function haveActiveCall(LineNumber: number) {
	...
	// alert(session.disposed)
	if (!session.disposed) return;
	if (!session.data.confcalls) {
		// If no conf call finish
	...
	console.log(lineObj, "..............................");
	if (activeConfCall == true) return; // If active conf call do nothing
	console.log(activeConfCall, "activeConfCallactiveConfCall");
	for (const conf of session.data.confcalls) {
		// Make sure to hungup all conf call
		if (conf.session._state !== "Terminated") {
...
const sip = {
	...
	volumeLevel: (LineNumber: number, volumeLevel: string) => {
		...
		store.dispatch({
			type: "sip/answeredCalls",
			payload: { action: "volumeLevel", data: { lineNum: LineNumber, volumeLevel: volumeLevel } },
		});
		...
		if (typeof remoteAudio !== "undefined" && typeof remoteAudio.sinkId !== "undefined") {
			remoteAudio
				...
				.then(function () {
					...
					store.dispatch({
						type: "sip/answeredCalls",
						payload: { action: "callSpeakerDevice", data: { lineNum: LineNumber, callSpeakerDevice: value } },
					});
				})
				.catch(function (e) {
					console.warn("Error using setSinkId: ", e);
			...
			if (true) {
				...
				navigator.mediaDevices
					...
					.then(function (newStream) {
						...
						store.dispatch({
							type: "sip/answeredCalls",
							payload: { action: "callMicrophoneDevice", data: { lineNum: LineNumber, callMicrophoneDevice: value } },
						});
					})
					.catch(function (e) {
	...
	ringtone: (LineNumber: number, status: boolean) => {
		...
		if (session.data.ringerObj) {
			if (status == true) {
				...
				session.data.ringerObj.currentTime = 0;
				store.dispatch({
					type: "sip/ringingInboundCalls",
					payload: { action: "ringtoneOff", data: lineObj.LineNumber },
				});
			} else {
				session.data.ringerObj.play();
				store.dispatch({
					type: "sip/ringingInboundCalls",
					payload: { action: "ringtoneOn", data: lineObj.LineNumber },
				});
			}
		}
	},
