...
interface IAddContact {
	close: (disp: boolean) => void;
}

const AddContact: React.FC<IAddContact> = ({ close }) => {
					...
					<div className={styles.box}>
						...
						<div className={`${styles.inputBox}`}>
							...
							<input
								...
								value={number}
								onChange={(event) => {
									dispatch(setCallNumber(event.target.value));
									contactData && setContactData({ ...contactData, phone: number });
								}}
							/>
						</div>
						<div className={`${styles.inputBox}`}>
